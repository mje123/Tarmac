...
export async function GET(req: NextRequest) {
  ...
  if (postId.startsWith('auto-')) {
    const weekStart = postId.replace('auto-', '')
    const { data: realPost } = await admin.from('qotw_posts').select('id').eq('week_start', weekStart).maybeSingle()
    if (!realPost) return NextResponse.json({ responses: [] })
    postId = realPost.id
  }
  ...
  for (const c of commentCounts || []) commentMap[c.response_id] = (commentMap[c.response_id] || 0) + 1
