...
import { AlertTriangle, ThumbsUp, Flag, Trash2, ChevronDown, Archive, Send, Loader2, X } from 'lucide-react'
...
export default function DebriefPage() {
  ...
  const loadComments = useCallback(async (accidentId: string) => {
    const supabase = createClient()
    const { data } = await supabase
  ...
  useEffect(() => {
    if (accident) loadComments(accident.id)
  }, [accident, loadComments, sort])

  async function submitComment(e: React.FormEvent) {
    e.preventDefault()
    if (!body.trim() || !accident) return
    setSubmitting(true)
    setError('')
    try {
      const res = await fetch('/api/accidents/comment', {
        ...
        body: JSON.stringify({ accidentId: accident.id, body }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Failed to post'); return }
      setBody('')
      await loadComments(accident.id)
    } finally {
      setSubmitting(false)
    }
  }
    ...
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <div className="rounded-xl p-8 text-center" style={{ background: 'var(--surface-1)', border: '1px solid var(--border-1)' }}>
          ...
          <h2 className="text-lg font-bold text-white mb-2">No Active Debrief</h2>
          <p className="text-sm" style={{ color: 'var(--text-sec)' }}>The next scenario is being prepared. Check back in a couple of days.</p>
          <Link href="/debrief/archive" className="inline-block mt-4 text-sm font-semibold" style={{ color: 'var(--debrief-accent)' }}>
            Browse past debriefs →
          </Link>
        </div>
      </div>
    )
    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <div className="rounded-xl p-5 mb-5" style={{ background: 'var(--surface-1)', border: '1px solid var(--border-1)' }}>
        ...
        <p className="text-xs mt-4" style={{ color: 'var(--text-ter)' }}>
          Powered by NTSB data · Anonymized for educational discussion · New scenario every other day
        </p>
      </div>
